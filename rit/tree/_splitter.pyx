...
ctypedef cset[SIZE_t] CSET
ctypedef cset[SIZE_t].iterator CSET_ITERATOR
ctypedef cvector[SIZE_t] CVECTOR
# ctypedef cvector[SIZE_t].iterator CVECTOR_ITERATOR
# === RIT === #
...
cdef class Splitter:
    ...
    def __cinit__(self, Criterion criterion, SIZE_t max_features,
                  ...
                  double rit_alpha, object random_state):
        """
        Parameters
        ----------
        criterion : Criterion
...
# === RIT === begin
cdef void swap_features(
    const CSET_ITERATOR& it_j, # Iterator of the index of the drawn feature in 'features' array; 
                               # the index is stored in stored in 'in_tree_remained'. 
    SIZE_t f_j,                # The value of the index, used if it is not in 'in_tree_remained'. 
    SIZE_t n_bound,            # The index of a bound index in 'features', e.g. n_drawn_constants, n_total_constants, and f_i.
    CSET& in_tree_remained,    # Stores indices of the features that have been used
                               # in the splits of the current tree
                               # and have not been drawn yet. 
    CVECTOR& in_tree_drawn     # Stores in-tree feature indices, 
                               # which have already been drawn
) noexcept nogil:
    it_bound = in_tree_remained.find(n_bound)
    if it_j != in_tree_remained.end():
        if it_bound != in_tree_remained.end():
            in_tree_remained.erase(it_bound)
        else:
            in_tree_remained.erase(it_j)
        in_tree_drawn.push_back(n_bound)
    else:
        if it_bound != in_tree_remained.end():
            in_tree_remained.erase(it_bound)
            in_tree_remained.insert(f_j)
...
cdef inline int node_split_best(
    ...
    in_tree_remained.insert(
        splitter.features_in_tree.begin(), splitter.features_in_tree.end())
    cdef CSET_ITERATOR it_j
    
    # Indices in 'features' array of the features 
    # that are already in the decision tree (were used in plits before) 
    # and have been already processed as either constant or split features.
    # In-tree features "moves" from 'in_tree_remained' to 'in_tree_drawn'
    # and their indices changes (due to swap)
    cdef CVECTOR in_tree_drawn

    # Flag whether the processed feature is already in tree
    cdef SIZE_t in_tree
    cdef SIZE_t best_k
    ...
    # with gil:
    #     print('n_known_constants:', n_known_constants)
    # === RIT === end
    ...
    #
    # Skip the CPU int ensive evaluation of the impurity criterion for
    # features that were already detected as constant (hence not suitable
    # for good splitting) by ancestor nodes and save the information on
    # newly discovered constant features to spare computation on descendant
    # nodes.
    while f_i > n_total_constants:  # Stop early if remaining features are constant 
        # Loop invariant: elements of features i n
        ...
        if (n_visited_features < max_features or \
           ...
           n_visited_features <= n_found_constants + n_drawn_constants):
            ...
            if f_j >= n_known_constants:
                # set f_j in the interval [n_total_constants, f_i[
                f_j += n_found_constants
                # with gil:
                #     if n_found_constants > 0:
                #         print('f_j>=n_found_constants=', n_found_constants, 'f_j:', f_j, end=' ')

            # Now f_j is in either [n_drawn_constants, n_known_constants[ 
            # or [n_total_constants, f_i[;
            # intervals [:n_drawn_constants[ and [n_known_constants, n_total_constants[ are ignored

            # === RIT === begin
            # Try to find f_j among features which are already in the tree 
            it_j = in_tree_remained.find(f_j)
            # check if the feature was already used in the splits of the tree
            in_tree = 1 if it_j != in_tree_remained.end() else 0

            # if it_j != in_tree_remained.end():
            #     with gil:
            #         print('(', features[deref(it_j)], ')', end=' ')
            # else:
            #     with gil:
            #         print(features[deref(it_j)], end=' ')
        elif not in_tree_remained.empty(): 
            # Additional in-tree features
            it_j = in_tree_remained.begin()
            f_j = deref(it_j)
            in_tree = 2
            ...
            # with gil:
            #     print('remained f_j:', f_j, end=' ')
            # === RIT === end
        else:
            # # no features to process are remained
            # with gil:
            #     print('break')
            break
        ...
        if f_j < n_known_constants:
            ...
            # === RIT === begin
            swap_features(it_j, f_j, n_drawn_constants, in_tree_remained, in_tree_drawn) 

            # with gil:
            #     print('n_drawn_constants:', n_drawn_constants, 
            #           'features:', np.array(features),
            #           # 'in_tree_remained:', np.array(n_tree_remained), 
            #           # 'in_tree_drawn:', np.array(in_tree_drawn), 
            #           end='\n')
            # === RIT === end
        ...
        if (
            ...
            # === RIT === begin
            swap_features(it_j, f_j, n_total_constants, in_tree_remained, in_tree_drawn)
            # === RIT === end 
            ...
            #           # 'in_tree_remained:', np.array(n_tree_remained), 
            #           # 'in_tree_drawn:', np.array(in_tree_drawn), 
            #           'current_split.feature:', current_split.feature, end='\n')
        ...
        # === RIT === begin
        swap_features(it_j, f_j, f_i, in_tree_remained, in_tree_drawn)
        # === RIT === end
    ...
    # === RIT === begin
        if best_split.pos < end:  # best split found
            ...
            splits_fi.push_back(f_i)

    # fill in_tree feature indices from in_tree_remained and in_tree_drawn back to features_in_tree
    splitter.features_in_tree = in_tree_drawn  # copy
    splitter.features_in_tree.insert(
        splitter.features_in_tree.end(),
        in_tree_remained.begin(), in_tree_remained.end())

    if splits.size() > 0:
        # with gil:
        #     print('new features:', end=' ')
        ...
        if best_k < splits.size():
            ...
            if splits_intree[best_k] == 0:  # not in-tree
                splitter.features_in_tree.push_back(splits_fi[best_k])
                # with gil:
                #     print(best_split.feature)
            elif splits_intree[best_k] == 1:
                pass
                # with gil:
                #     print('(', best_split.feature, ')')
            else:
