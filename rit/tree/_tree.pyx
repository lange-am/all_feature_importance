...
cdef INT32_t _MAX_INT32 = MAX_INT32
# === RIT ===
    ...
    cpdef build(
        ...
        with nogil:
            ...
            while not builder_stack.empty():
                ...
                if not is_leaf:
                    # === RIT === begin
                    # Left and right nodes selected randomly
                    # for the same contribution to in_tree features
                    if rand_int(0, 2, &splitter.rand_r_state):
...
cdef class BestFirstTreeBuilder(TreeBuilder):
    ...
    cpdef build(
        ...
        cdef SIZE_t init_capacity = \
            max_split_nodes + max_leaf_nodes if max_leaf_nodes < _MAX_INT32 else 2047
        # === RIT ===
        tree._resize(init_capacity)
        ...
        with nogil:
            ...
            while not frontier.empty():
                ...
                if is_leaf:
                    ...
                    # Add nodes to queue
                    if rand_int(0, 2, &splitter.rand_r_state): # ==== RIT
                        _add_to_frontier(split_node_left, frontier)
    ...
    cdef inline int _add_split_node(
        ...
        if not is_leaf:
            # with gil:
            #     print('depth:', depth, 'left' if is_left else 'right', end=' ')
            #     print('features before node_split():', np.array(splitter.features))
            ...
            # with gil:
                # print('split:', split, 'n_constant_features:', n_constant_features)
                # print('features:', np.array(splitter.features))
                # print('features_in_tree idx:', np.array(splitter.features_in_tree))
                # print('features_in_tree:', end =" ")
                # for i in splitter.features_in_tree:
                #     print(splitter.features[i], end =" ")
                # print('\n')
...
cdef class Tree:
    ...
    cpdef compute_feature_importances(
        ...
        normalize : bool, default=True
            normalize the importances of the features used in a tree such that they sum up to 1.
        fillna: bool, default=True # ==== RIT
            fill the importances of the features that were not used in a tree by zero 
        ...
        cdef bint fillna_ = fillna

        cdef cnp.float64_t[:] importances = np.full(
            self.n_features, 0. if fillna else np.nan
        )
        ...
        with nogil:
            while node != end_node:
                if node.left_child != _TREE_LEAF:
                    ...
                    # ==== RIT 
                    if not fillna_ and isnan(importances[node.feature]):
        ...
        for i in range(self.n_features):
            if fillna or not isnan(importances[i]):  # ==== RIT
        ...
        if normalize:
            if fillna:
                ...
                normalizer = np.nansum(importances)  # ==== RIT
