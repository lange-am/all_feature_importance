...
from sklearn.tree._tree import _build_pruned_tree_ccp, ccp_pruning_path
...
# === RIT === begin
MAX_POW2 = int(np.log2(MAX_INT32)) # max N such that 2**N < MAX_INT32
# === RIT === end
...
class BaseDecisionTree(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    _parameter_constraints: dict = {
        ...
        "max_leaf_nodes": [
            Interval(Integral, -1, 0, closed="both"), # === RIT ===  
            Interval(Integral, 2, None, closed="left"), 
            None,
        ],
        "min_impurity_decrease": [Interval(Real, 0.0, None, closed="left")],
        "ccp_alpha": [Interval(Real, 0.0, None, closed="left")],
        "rit_alpha": [Interval(Real, None, 1.0, closed="neither"), None],
    }
    ...
    def _fit(
        ...
        max_depth = MAX_INT32 if self.max_depth is None else self.max_depth
        ...
        # === RIT === begin
        # line removed from initial sklearn:
        ...
        if (self.max_leaf_nodes is not None) and (self.max_leaf_nodes < 0):
            ...
            if (max_leaf_nodes == 0 or max_leaf_nodes is None):
                max_leaf_nodes = MAX_INT32 if max_depth > MAX_POW2 else 2**max_depth
            # === RIT === end
...
class DecisionTreeClassifier(ClassifierMixin, BaseDecisionTree):
    ...
    max_leaf_nodes : int, default=None
        ...
        If 0 or None then unlimited number of leaf nodes,
        in particular 2**``max_depth`` if ``max_depth`` is given.
        If -1 then depth-first method is used.
...
# === RIT === begin
    rit_alpha : double, default=None
        Redundancy Insensitive Tree alpha.
        Accceptable reduction of impurity improvement (e.g. 0.1) when choosing 
        a feature already used in the tree earlier, 
        instead of the best feature from those sampled during the current split.
        If -np.inf or None then classical decision tree is built, 
        i.e. the best feature is selected only among those sampled according to 
        ``max_features``.

    fillna : bool, default=True
        If True then the importance of a feature that was not occured in the tree
        is set to zero (sklearn default), otherwise is set no np.nan. 

    normalize_importance : bool, default=True
        If True then the feature importances (impurity improvements) are normalized 
        (divided by the sum of all importances - sklearn default),
...
# === RIT === end
    ...
    def fit(self, X, y, sample_weight=None, check_input=True):
        ...
        #     'rit_alpha', self.rit_alpha, 
        #     'fillna', self.fillna, 
        #     'normalize_importance', self.normalize_importance)
